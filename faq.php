...
$color = $_POST['color'];
$size = $_POST['size'];
$new_quantity = $_POST['new_quantity']; // User updated quantity
$user_id = $_POST['user_id']; // User ID or session ID

// Fetch available stock from product_variants
$variant_check_query = "SELECT available_stock FROM product_variants WHERE product_id = ? AND color = ? AND size = ?";
$stmt = $conn->prepare($variant_check_query);
...
if ($data) {
    ...
    if ($new_quantity <= $data['available_stock']) {
        // Update cart quantity
        $update_query = "UPDATE cart SET quantity = ? WHERE user_id = ? AND product_id = ? AND color = ? AND size = ?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("iisiss", $new_quantity, $user_id, $product_id, $color, $size);
        $stmt->execute();
        echo json_encode(['status' => 'success']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Only ' . $data['available_stock'] . ' item(s) available in stock.']);
    }
} else {
